...
class EnvStructureVisitor(ASTVisitor):
    """
    ...
    def visit_cell_ref(self, ast, vars, env_structure):
        ...
        ast.set_env_structure(env_structure)
        return ast
    ...
    def visit_app(self, ast, vars, env_structure):
        ...
        ast.rator.visit(self, vars, env_structure)
        for r in ast.rands:
            r.visit(self, vars, env_structure)
    ...
    def visit_lambda(self, ast, vars, env_structure):
        ...
        ast.set_enclosing_env_structure(env_structure)
        ast.set_env_structure(sub_env_structure)
        ast.init_body_pruning(sub_env_structure, body_remove_num_envs)
        ast.init_mutable_var_flags(need_cell_flags)
        return ast
    ...
    def visit_letrec(self, ast, vars, env_structure):
        ...
        for rhs in ast.rhss:
            rhs.visit(self, new_vars, sub_env_structure)
        ...
        ast.set_args(sub_env_structure)
        ast.init_body_pruning(sub_env_structure, body_remove_num_envs)
        return ast
    ...
    def visit_let(self, ast, vars, env_structure):
        ...
        for i, rhs in enumerate(ast.rhss):
            ...
            rhs.visit(self, vars, env_structures[i])
            offset += count
        ...
        ast.set_args(sub_env_structure)
        ast.set_remove_num_envs(remove_num_envs)
        ast.init_body_pruning(body_env_structure, body_remove_num_envs)
        ast.init_mutable_var_flags(need_cell_flags)
        return ast
    ...
    def visit_begin(self, ast, vars, env_structure):
        ...
        ast.init_body_pruning(env_structure, body_removes)
        return ast
...
def compute_env_structures(ast, visitor=None):
    if visitor is None:
        visitor = EnvStructureVisitor()
