...
(define (extract start-mod-path cache
  ...
  (when (linklets-are-source-mode? linklets)
    ...
    (define linklet (compile-linklet pruned-linklet-expr))

    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    
    (define t (open-output-bytes))
    (write (hash->linklet-bundle (hasheq 0 linklet)) t)

    (define byte-file (string-append (if (path? print-extracted-to) (path->string print-extracted-to) print-extracted-to)
                                     ".bytecode"))
    ;(displayln (format "CANER : writing bytecode to : ~a" byte-file))

    (call-with-output-file* byte-file #:exists 'replace
      (lambda (o) (write-bytes (get-output-bytes t) o)))
    ...
    (define json-ast
      (handle-linkl
       ...
       "expander"
       "rkt"
       "/home/cderici/racketland/tmp/racket/racket/src/expander/extract/"))

    (define json-file (string-append (if (path? print-extracted-to) (path->string print-extracted-to) print-extracted-to)
                                     ".pycket_ast"))
    ...
    (call-with-output-file*
      ...
      #:exists 'replace
      (lambda (o)
        #;(write-bytes (get-output-bytes t) o)
        (write-json json-ast o)))
